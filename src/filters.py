...
def _load_keywords(config: dict) -> tuple[list[str], list[str], list[str]]:
    return (
...
def matches(job: Job, keywords_config: dict) -> bool:
    climate_kw, management_kw, ic_kw = _load_keywords(keywords_config)
    ...
    is_management = _contains_any(job.title, management_kw)
